...
pub trait BitmapSlice: Bitmap + Clone + Debug {
    type S<'memory>: BitmapSlice;
}
...
pub trait Bitmap {
    ...
    fn slice_at<'memory>(&self, offset: usize) -> Self::S<'memory>;
}
...
pub type MS<'memory, M> =
    <<<M as GuestMemory>::R<'memory> as GuestMemoryRegion<'memory>>::B as Bitmap>::S<'memory>;
